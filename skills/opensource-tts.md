...
>
> **۱. سقفِ حافظه.** هر اجرای مدلِ محلی از پشتِ **`run-capped`** (سقف ۸–۱۲GB) و با
> **dtype ِ ۱۶ بیتی** — بخشِ «سقفِ حافظه» در انتهای همین فایل. یک `torch.float32` ِ
> جاافتاده دو بار مکِ مالک را هنگ کرده است.
>
> **۲. edge-tts ممنوع.** هرگز پیشنهادش نده — مالک آن را ردّ کرده است. جدول و مثال‌های
> زیر فقط مرجعِ فنی‌اند، نه توصیه. کندیِ موتورِ باکیفیت را با سخت‌افزار حل کن، نه با
> افتِ کیفیت.
