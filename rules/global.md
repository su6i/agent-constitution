...
Details: `rules/050-session-start.md`
...
- **Initialization:** ALWAYS follow `workflows/init-project.md` for new projects.
...
- **Documentation:** ALWAYS follow `workflows/documentation.md` for writing docs.
- **AI Logic:** ALWAYS follow `workflows/ai-optimization.md` for implementing AI features.
- **QA & Git:** ALWAYS follow `workflows/quality-assurance.md` for testing and commits.
- **Showcase:** ALWAYS follow `workflows/social-media-showcase.md` after completion.
...
- **Centralized Configuration (Single Source of Truth):** For multi-language projects (Bash + Python), encoding parameters, media standards, and shared constants MUST be stored in `lib/config/*.json`. Both languages read from the SAME file via language-specific API layers (`media_config.py` for Python, `get_media_config()` in Bash). This prevents code duplication and maintains DRY principles. See `skills/centralized-config-pattern.md` for implementation details.
...
- **Phase 1 (Internal Review):** Read all relevant `skills/*.md` files and existing codebase logic. If a working pattern exists (e.g., Zsh completion patterns, Python data processing), you MUST adapt it instead of creating a new one.
