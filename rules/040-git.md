...
4. Update all relevant docs — **before** staging the commit:
   - `README.md`, `docs/ARCHITECTURE.md` (the single technical/design doc — see `045`), `CHANGELOG.md`, `AGENTIC-CODING-SETUP.md`
...
A second, machine-global backstop covers repos that never installed the
per-repo hook above: `templates/hooks/guard-data-leak`, installed once via
`core.hooksPath` (see README §Machine-Global Git Hooks), runs the same class
of secret/PII/oversized-document scan **unconditionally in every repo on the
machine** — see `rules/035-data-vault.md`.

## Retroactive (Full-Tree) Security Scan (Non-Negotiable)
...
## Forbidden Working-Tree Commands (Non-Negotiable)
